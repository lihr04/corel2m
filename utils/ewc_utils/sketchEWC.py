...
#                 self._jacobian_matrices[n].data[r] += (1-self.alpha) / math.sqrt(n_data) * p.grad.data 
    ...
    def consolidate(self, dataloader: torch.utils.data.DataLoader, labels=None):
        ...
        jacobian_matrices = torch.cat((math.sqrt(self.alpha) * self._jacobian_matrices,
                                       math.sqrt(1 - self.alpha) * self.calculate_jacobian(dataloader)))
        _, s, v = torch.svd_lowrank(jacobian_matrices, q=self.n_sketch)
        # W^T W, W=U*S*V^T
        # W^T W = V*S*S*V^T
        self._jacobian_matrices = torch.matmul(torch.diag(s), v.t())
#         print(torch.sum(s[0:self.n_sketch] ** 2) / torch.sum(s**2))
#         self._jacobian_matrices = jacobian_matrices
